...
## E. Fraktal Router / FVDB (Anlamsal Yönlendirme)
Standart sistemler tüm girdiyi bir kerede analiz eder. Maxwell Engine ise FVDB (Fraktal Vektör Veritabanı) mantığını kullanarak:
1. Gelen metni 64 token'lık parçalara (chunk) böler.
2. Her bir parçanın kendi içindeki **Lokal Entropisini (Fraktal Ağırlığını)** hesaplar.
3. En yüksek entropiye sahip (en kaotik) parçayı bulur ve LLM'in dikkatini (Attention) doğrudan o noktaya matematiksel olarak sabitler. 
Böylece model koca metin içinde kaybolmaz; zafiyetin kalbine yönlendirilir.

## F. Termodinamik Filtre (Doğal Logaritma Ortalaması)
...
4. Bu değeri 0.0 ile 1.0 arasına sıkıştırarak (Normalize ederek) son **Kritiklik Skorunu** (Global Sistem Entropisi) üretir.

## G. `I.w > 0.7` mi? (Kritiklik Eşiği Kontrolü)
...
## H & I. Çatallanma (Bifurcation) ve Gürültü (Noise) Ayrımı
* **(I) HAYIR (Düşük Enerji - Gürültü Filtrelendi):** Eğer skor 0.7'nin altındaysa (örneğin 0.2), sistem bunun sıradan bir kod hatası, ufak bir yazım yanlışı veya önemsiz bir boşluk (Gürültü) olduğuna karar verir. "Uyarı Yorgunluğunu" engellemek için bu ufak hataları umursamaz.
* **(H) EVET (Yüksek Enerji - Çatallanma Tespit Edildi):** Eğer skor 0.7'yi aşarsa (örneğin 0.95), sistem az önce devasa bir yapısal kaos, bir güvenlik zafiyeti (Backdoor) veya sistemi çökertecek bir mantık hatası bulduğunu anlar. Sistemin olağan akışının kırıldığı bu noktaya **Bifurcation (Çatallanma)** denir.

## J. Nihai Rapor Çıktısı (Sıfır Halüsinasyon)
...
## E. Fractal Router / FVDB (Semantic Routing)
Standard systems analyze the entire input at once. Maxwell Engine uses FVDB (Fractal Vector Database) logic to:
1. Divide the incoming text into 64-token chunks.
2. Calculate the **Local Entropy (Fractal Weight)** within each chunk.
3. Find the chunk with the highest entropy (most chaotic) and mathematically lock the LLM's Attention directly to that point.
This way, the model does not get lost in a massive text; it is routed to the very heart of the vulnerability.

## F. Thermodynamic Filter (Average Natural Logarithm)
...
4. It compresses (Normalizes) this value between 0.0 and 1.0 to produce the final **Criticality Score** (Global System Entropy).

## G. Is `I.w > 0.7`? (Criticality Threshold Check)
...
## H & I. Bifurcation and Noise Separation
* **(I) NO (Low Energy - Noise Filtered):** If the score is below 0.7 (e.g., 0.2), the system decides this is an ordinary code error, a minor typo, or insignificant whitespace (Noise). It ignores these minor errors to prevent "Alert Fatigue".
* **(H) YES (High Energy - Bifurcation Detected):** If the score exceeds 0.7 (e.g., 0.95), the system realizes it just found massive structural chaos, a security vulnerability (Backdoor), or a logic error that will crash the system. This point where the system's normal flow breaks is called a **Bifurcation**.

## J. Final Output Report (Zero Hallucination)
